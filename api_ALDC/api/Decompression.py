...
class Decompression(Resource):
    def __init__(self):
        latent_dim = 128
        self.model = tf.keras.models.load_model('s2s_c.h5')
    ...
    def post(self):
        if request.method == 'POST':
            ...
            if type == 'txt':
                ...
                d = np.zeros((len(data),47,131))
                for idx,i in enumerate(data):
                    for t,char in enumerate(i):
                        d[idx,t,LETTER_IDX[char]] = 1.0
                    
                pred = self.decompress(d)
                z = ''
                ...
                with open(f'decompressed files/{filename}','w+',encoding='utf-8') as f:
                    f.write(z)
            ...
            if type == 'img':
                ...
                x = []
                for i in range(0,len(img),47):
                    x.append(img[i:i+47])
                ori = np.zeros((len(x),47,131))
                for idx,i in enumerate(x):
                    for t,char in enumerate(i):
                        if char != '':
                            ori[idx,t,int(char)] = 1.0

                pred = self.decompress(ori)
                z = ''
